...
|---|------|-----|------------------|-----|-------|
| 15 | 2026-06-23 | **A quiz question graded a *wrong* answer as correct (colliding options).** Found by checking that every quiz answer the grader accepts is the intended one. Lesson **i07** ("Which csv reader returns rows as dicts?") had options `['csv.reader', 'csv.DictReader', 'csv.dictreader', 'csv.NamedReader']` with `correct_answer='csv.DictReader'`. `grade_answer` normalizes (lowercases) before comparing, so the correct option and the distractor `csv.dictreader` both normalized to `csv.dictreader` — selecting the **wrong** option (position 3) graded as **correct**. | `test_multiple_choice_options_distinct_after_normalization` (+ `..._correct_option_position_grades_true`, `test_text_answer_questions_grade_their_own_correct_answer`) | Replaced the colliding distractor with `csv.DictWriter` (a real csv class that *writes* dicts — plausible but clearly wrong for reading, and normalizes distinctly). | `src/python_mastery_cli/curriculum/intermediate.py`, `tests/test_curriculum.py` |
| 14 | 2026-06-23 | **Two self-contained drills' reference solutions didn't match their own `expected_output`, breaking "Run & check".** Found by running every drill's reference solution and comparing to its `expected_output`. **b21** ("Slice & Dice a Word"): the solution is correct Python but `expected_output` (and the inline comments) had the wrong slice results — `'fantastic'[-3:]` is `'tic'` (not `'ic!'`) and `[::2]` is `'fnatc'` (not `'fnai'`), so a learner who solved it correctly was told "doesn't match". **i02** ("Word Frequency Table"): the solution iterated `set(words)`, whose iteration order is hash-randomised per process, so the dict printed in a **non-deterministic** order that rarely matched the fixed `expected_output` (verified: two runs produced different orders). | `test_self_contained_drill_solution_matches_expected_output[b21]`, `[i02]` (runs each solution, asserts exact match) | b21: corrected `expected_output` to `fan/tic/fnatc/True` and fixed the misleading comments. i02: iterate `words` (first-seen insertion order) instead of `set(words)` so output is deterministic and matches; aligned the hints. Verified deterministic across `PYTHONHASHSEED` 0/1/42/123/999. | `src/python_mastery_cli/curriculum/michigan_basics.py`, `src/python_mastery_cli/curriculum/intermediate.py`, `tests/test_curriculum.py` |
...
### 2026-06-23 — quiz answer correctness
- **Bugs found & fixed: 1** (Bug #15: i07 had two options that collide under the
  grader's normalization, so picking the wrong distractor graded as correct).
- Method: for all 299 quiz questions, checked that the stored answer grades
  correct and (for MC) that exactly one option matches and options are distinct
  after normalization. Flagged i07 (collision) and 3 numeric-MC questions
  (b06/b07/i03) — the latter were **not** bugs: they're answered by position
  number in the UI and the correct position grades correctly; only feeding the
  literal digit text (not an MC UI path) "failed". Did not manufacture fixes.
- Fix: replaced i07's colliding distractor `csv.dictreader` with `csv.DictWriter`.
- 3 reproducing/guard tests. **295 tests, 100% line + branch coverage.** Bug tally: 15.

### 2026-06-23 — drill reference-solution correctness
- **Bugs found & fixed: 1** (Bug #14: two drills' reference solutions didn't
  reproduce their own `expected_output` — b21 wrong slice expectations, i02
  non-deterministic `set()` ordering). Breaks the "Run & check" feedback loop.
- Method: ran every drill's reference solution in the subprocess runner and
  compared stdout to `expected_output`. Triaged 17 "errors" (third-party libs
  not installed, `input()` EOF — all expected) and 8 mismatches (6 were
  intentional placeholders like "varies by directory"; b21 + i02 were real).
- Fix: corrected b21's expected output/comments; made i02 deterministic by
  iterating the list instead of a set. Confirmed stable across hash seeds.
- 2 reproducing tests. **292 tests, 100% line + branch coverage.** Bug tally: 14.
...
- Method: exploratory testing of the recently-added `runner.py` (real subprocesses
  with binary output and `input()`), since it only caught `TimeoutExpired`.
- Fix: `errors="replace"` + `stdin=subprocess.DEVNULL` on the subprocess call.
- 2 reproducing tests. **285 tests, 100% line + branch coverage.** Bug tally: 10.
